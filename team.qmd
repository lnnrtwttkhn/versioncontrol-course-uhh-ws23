---
title: "Team"
toc: true
toc-title: "Team"
---

```{r, include=FALSE}
knitr::opts_chunk$set(echo=FALSE, message=FALSE, warning=FALSE)
library("magrittr")
```

```{r}
variables <- yaml::read_yaml("_variables.yml")
dt <- data.table::rbindlist(variables$team, fill=TRUE) %>%
  data.table::melt(., id.vars = c("id")) %>%
  .[variable == "email", ":=" (
    icon = "envelope", text = "Email", href = paste0("mailto:", value), aria_label = "Email"
  )] %>%
   .[variable == "website", ":=" (
    icon = "house-user", text = "Website", href = value, aria_label = "Website"
  )] %>% 
   .[variable == "github", ":=" (
    icon = "brands github", text = "Website", href = value, aria_label = "GitHub"
  )] %>% 
   .[variable == "twitter", ":=" (
    icon = "brands twitter", text = "Twitter", href = value, aria_label = "Twitter"
   )] %>% 
  .[variable == "mastodon", ":=" (
    icon = "brands mastodon", text = "Mastodon", href = value, aria_label = "Mastodon"
  )] %>% 
  .[variable == "scholar", ":=" (
    icon = "graduation-cap", text = "Google Scholar", href = value, aria_label = "Google Scholar"
  )] %>% 
  .[variable == "linkedin", ":=" (
    icon = "brands linkedin", text = "LinkedIn", href = value, aria_label = "LinkedIn"
  )] %>%
  .[complete.cases(.[, "value"])] %>%
  .[, button := sprintf('{{< fa %s >}} [%s](%s){target="_blank"}', icon, text, href)] %>%
  .[, button_icon := sprintf('[{{< fa %s >}}](%s){target="_blank"}', icon, href)]

create_grid = function(buttons) {
  content <- sprintf(" %s ", buttons)
  n_char <- unlist(lapply(content, function(x) nchar(x)))
  content <- paste0("|", paste(content, collapse = "|"), "|")
  n_buttons <- length(buttons)
  border_start <- paste0("+", strrep("-", n_char[1]), "+")
  border_rest <- paste0(strrep("-", n_char[2:length(n_char)]), rep("+", n_buttons - 1))
  borders <- paste(c(border_start, border_rest), collapse = "")
  text = cat(borders, content, borders, sep = "\n")
  return(text)
}
```

## Project Leader & Instructor

::: {.grid}

::: {.g-col-4}
![](photo_wittkuhn_portrait.jpg){fig-alt="A portrait photo of Lennart Wittkuhn."}
:::

::: {.g-col-8}

### Dr. Lennart Wittkuhn

Lennart is a **Postdoctoral Researcher** and **Lab Manager** in the [Research Group "Mechanisms of Learning and Change"](https://www.psy.uni-hamburg.de/en/arbeitsbereiche/lern-und-veraenderungsmechanismen.html) at the [University of Hamburg](https://www.uni-hamburg.de/en.html) and the [Max Planck Research Group NeuroCode](https://www.mpib-berlin.mpg.de/research/research-groups/mprg-neurocode) at the [Max Planck Institute for Human Development](https://www.mpib-berlin.mpg.de/en) in Berlin.
In his scientific work, Lennart investigates **neural replay of task representations** in the human brain using functional magnetic resonance imaging (fMRI).
He is also passionate about **open science**, **coding**, **research data management**, **reproducibility** and tools that improve the scientific workflow.

```{r, results='asis', echo=FALSE}
buttons <- dt %>% .[id == "lennart"] %>% .$button_icon %>% create_grid()
```
:::

:::

## Scientific Advisor

::: {.grid}

::: {.g-col-8}
### Prof. Dr. Nicolas Schuck
Nico is a professor of Psychology and principal investigator of the [Research Group "Mechanisms of Learning and Change"](https://www.psy.uni-hamburg.de/en/arbeitsbereiche/lern-und-veraenderungsmechanismen.html) at the [University of Hamburg](https://www.uni-hamburg.de/en.html) and the [Max Planck Research Group NeuroCode](https://www.mpib-berlin.mpg.de/research/research-groups/mprg-neurocode) at the [Max Planck Institute for Human Development](https://www.mpib-berlin.mpg.de/en) in Berlin.

```{r, results='asis', echo=FALSE}
buttons <- dt %>% .[id == "nico"] %>% .$button_icon %>% create_grid()
```
:::

::: {.g-col-4}
![](photo_schuck_portrait.jpg){fig-alt="A portrait photo of Nicolas Schuck."}
:::

:::

## Student Research Assistant

::: {.grid}

::: {.g-col-4}
![](https://upload.wikimedia.org/wikipedia/commons/thumb/5/59/User-avatar.svg/2048px-User-avatar.svg.png){fig-alt="A portrait photo of Nicolas Schuck."}
:::

::: {.g-col-8}
### You?
We are looking for a **student research assistant** (SHK/WHK)!

- {{< fa list-check >}} **Tasks:** Supporting the teaching project (preparation of teaching materials, evaluation, project reporting, etc.)
- {{< fa business-time >}} **Duration:** 01.06.2023 to 31.03.2024 (10 months)
- {{< fa clock >}} **Hours:** 32 hours per month
- {{< fa money-check-dollar >}} **Salary:** SHK: 12.00 € / hour | WHK: 13.95 € / hour

Details can be found [here](https://lennartwittkuhn.com/ddlitlab/ausschreibung_whk.html) (in German).

:::
